...
describe('Fields API', () => {
    ...
    return addAuthentication(request(server.getHttpServer())
      ...
      .expect(
        (res)=>{
          ...
          expect(res.body.description).toEqual('First Name');
        }
      );
  });
  ...
  it(`Get List of fields with fields /api/fields?fields=id,name,description (GET)`, () => {
    return addAuthentication(request(server.getHttpServer())
      .get(`/api/fields?fields=id,name,description`))
      .expect(
        (res)=>{
          expect(res.body.pager != undefined).toEqual(true);
  ...
  it(`Get List of fields /api/fields?filter=name:eq:firstname (GET)`, () => {
    return addAuthentication(request(server.getHttpServer())
      .get(`/api/fields?filter=name:eq:firstname`))
      .expect(
        (res)=>{
          expect(res.body.pager != undefined).toEqual(true);
          expect(res.body.fields != undefined).toEqual(true);
      ...
      .send({
        ...
        "recordValues": [
          ...
          "field": {
            "id": "5289e934c02a1",
            "name": "surname",
            "description": "Compulsory, Employee Surname, Surname keeps history, update it through history.",
            "caption": "Surname"
            }
            },
            {
          ...
          "field": {
            "id": "5289e934a36d7",
            "name": "edu_evel",
            "description": "Compulsory, Employee`s Highest education level, Educational level keeps history, should be updated through history.",
